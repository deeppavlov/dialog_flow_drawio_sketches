...
do 
    ...
    FILE=$(echo $IN_FILE | sed -n 's/src\///p')
    OUT_FILE=$(echo $FILE | sed -n 's/.\/\(.*\)\.drawio/'$BUILD_DIR'\/\1\.'$TYPE'/p')
    OUT_DIR=$(echo $FILE | sed -n 's/.\/\(.*\)\/.*\.drawio/'$BUILD_DIR'\/\1/p')
    mkdir -p $OUT_DIR
    [ "$IN_FILE" ] && [ "$OUT_FILE" ] && $DRAWIO_CMD $IN_FILE -x -t -o $OUT_FILE -s 4;
done
